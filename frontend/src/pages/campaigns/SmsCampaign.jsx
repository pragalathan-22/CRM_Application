...
import { MessageSquareText, Send, Image as ImageIcon } from "lucide-react";
...
const SmsCampaign = () => {
  ...
  const handleSendCampaign = async () => {
    ...
    try {
      ...
      formData.append("numbers", JSON.stringify(mobileNumbers));
      if (image) formData.append("image", image);

      await axios.post("http://localhost:3000/send-sms-campaign", formData, {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      setStatus({
        type: "success",
        text: `✅ Campaign sent successfully to ${mobileNumbers.length} numbers!`,
      });
      setMessage("");
      setImage(null);
      setPreview("");
      ...
      setStatus({
        type: "error",
        text: "❌ Failed to send campaign. Please try again.",
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="p-6 sm:p-10 bg-white shadow-md rounded-lg max-w-5xl mx-auto">
      ...
      <p className="text-gray-600 mb-6">
        Manage and send your SMS or WhatsApp campaigns effectively. You can send
        a text, image, or both to all leads.
      </p>
      ...
      <div className="bg-gray-50 border rounded-lg p-6 mb-8">
        ...
        <textarea
          ...
          rows="4"
          placeholder="Write your SMS/WhatsApp message here..."
          className="w-full border rounded-lg px-4 py-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-green-500"
        />
